...
const EditPlace = ({ placeId, onCancel, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/places/${placeId}`)
      .then((res) => {
        if (!res.ok) {
          throw new Error('Failed to fetch destination details for editing.');
        }
        return res.json();
      })
      .then((data) => {
  ...
  const handleSubmit = (e) => {
    ...
    fetch(`/api/places/${placeId}`, {
      ...
      .then((res) => {
        if (!res.ok) {
          throw new Error('Failed to update tourist place. Check inputs.');
        }
        return res.json();
      })
      .then(() => {
  ...
  if (loading) {
    return (
      <div className="form-loader-container">
        ...
        <p className="loader-text">Loading destination details for editing...</p>
      </div>
    );
  }
    ...
    <div className="form-page">
      ...
      <div className="form-container">
        <header className="form-header">
          <h1 className="form-title">Edit Tourist Destination</h1>
          <p className="form-subtitle">Update destination details in the explorer directory.</p>
        </header>
